...
def bdAddAllFingerAttr(side):
	fingerAnim = cmds.ls(side.upper() + '_Hand_CON',type='transform')[0]
	fingerList = ['Index','Mid','Ring','Pinky','Thumb']
	...
	for finger in fingerList:
		...
		for attr in attrList:
			...
			fingerJoint = cmds.ls(side + finger + str(fingerPart) ,type='joint')[0]
			
			if attr == 'Spread':
				cmds.connectAttr(fingerAnim + "." + finger + '_' + attr,fingerJoint + '.rotateY')
			else:
				reverseVal = cmds.createNode('multiplyDivide',n = side + '_' + finger + '_' + str(fingerPart)) 
				cmds.setAttr(reverseVal + '.input2X',1)
				
				cmds.connectAttr(fingerAnim + "." + finger + '_' + attr,reverseVal + '.input1X')
				cmds.connectAttr(reverseVal + '.outputX',fingerJoint + '.rotateZ')
