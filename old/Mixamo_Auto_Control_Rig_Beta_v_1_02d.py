...
"""
Mixamo_AutoControlRig_Version = "Beta 1_02d"
    ...
    def createControl(self, type, rigType = "AutoRig", style = "circle", partialConstraint = 0, scale = (0.1,0.1,0.1), rotate = (0,0,0), offset = (0,0,0), estimateSize = True, color_ = None, name_ = None, flipFingers = False):
        ...
        if style == "circle":
            ...
            if estimateSize:
                ...
                mayac.scale(scale[0], scale[1], scale[2], control)
                if flipFingers:
                    mayac.rotate(rotate[0], 180, rotate[2], control, relative = True) #override for world-oriented rigs
                else:
                    mayac.rotate(rotate[0], rotate[1], rotate[2], control, relative = True) #override for world-oriented rigs
                mayac.move(offset[0], offset[1], offset[2], "%s.cv[0:7]" % (control), relative = True)
                mayac.makeIdentity(control, apply = True, t=1, r=1, s=1, n=0)
...
class DJB_Character():
        ...
        if not self.infoNode:
            ...
            if self.hulaOption:              
                ...
                if self.Root.Bind_Joint:
                    ...
                    print 'Executing DJB_CharacterNode'
                    self.Hips = DJB_CharacterNode("Hips", actAsRoot_ = 1)
                    print 'Finished Executing DJB_CharacterNode'
    ...
    def checkSkeletonProportions(self, incomingDataRootJoint):
        return True #temporarily disabling this because of false negatives
    ...
    def createExportSkeleton(self, keepMesh_ = False):
        ...
        for bodyPart in self.bodyParts:
            if "4" not in bodyPart.nodeName and "End" not in bodyPart.nodeName:
                ...
                if bodyPart.AnimData_Joint:
                    objectsOfInterest.append(bodyPart.AnimData_Joint)
                    connected = mayac.listConnections("%s.rotateX" % bodyPart.AnimData_Joint, d=False)
                    if connected:
                        objectsOfInterest.append(connected[0])
        objectsOfInterest.append(self.global_CTRL)
        ...
        if anythingToBake:
            for bodyPart in self.bodyParts:
                if bodyPart.Export_Joint:
                    #find curves
                    rxc = mayac.listConnections("%s.rotateX"%(bodyPart.Export_Joint))[0]
                    ryc = mayac.listConnections("%s.rotateY"%(bodyPart.Export_Joint))[0]
                    rzc = mayac.listConnections("%s.rotateZ"%(bodyPart.Export_Joint))[0]
                    mayac.filterCurve( rxc, ryc, rzc)
...
class MIXAMO_AutoControlRig_UI:
    def __init__(self):
        ...
        mayac.text( label='', align='left' )
        animationText = mayac.text( label='  Step 2: Import or reference your downloaded MIXAMO animation.', align='left' )
        mayac.text( label='(Referencing is for use if you intend to update the source animation fbx file.)', align='center' )
        ...
        self.browseMotions_button = mayac.button(label='Reference Animation', w=100, c=self.browseMotionsRef_function)
        mayac.text( label='', align='left' )
        mayac.text( label='  Step 3: Select the "Hips" joint of the imported motion and press copy or direct connection button.', align='left' )
        mayac.button(label='Copy Animation to Rig', w=100, c=self.copyAnimationToRig_function)
        mayac.button(label='Direct Connect Animation to Rig (Use with referenced animation)', w=100, c=self.connectAnimationToRig_function)
        mayac.text( label='', align='left' )
        mayac.text( label='', align='left' )
        ...
        mayac.text( label='', align='left' )
        self.bakeAnimation_button = mayac.button(label="Bake Animation to Controls (can't use with direct connect animation)", w=100, c=self.bakeAnimation_function)
        mayac.text( label='', align='left' )
        self.bakeAnimation_button = mayac.button(label="Clear Animation Controls (can't use with direct connect animation)", w=100, c=self.clearAnimation_function)
        mayac.text( label='', align='left' )
        mayac.text( label='', align='left' )
        self.deleteOrigAnimation_button = mayac.button(label="Delete Original Animation (can't use with direct connect animation)", w=100, c=self.deleteOrigAnimation_function)
        mayac.text( label='', align='left' )
    ...
    def setupControls_function(self, arg = None):
        global DJB_CharacterInstance
        DJB_populatePythonSpaceWithCharacter()
        print 'DJB_populatePythonSpaceWithCharacter executed'
        if not DJB_CharacterInstance:
            ...
            if not joints:
                ...
                print 'hula value %s'%str(hulaValue) 
                print 'Appending DJB_CharacterInstance'
                DJB_CharacterInstance.append(DJB_Character(hulaOption_ = hulaValue))
                print 'executing DJB_CharacterInstance[len(DJB_CharacterInstance)-1].fixArmsAndLegs()'
                DJB_CharacterInstance[len(DJB_CharacterInstance)-1].fixArmsAndLegs()
                print 'executing DJB_CharacterInstance[len(DJB_CharacterInstance)-1].makeAnimDataJoints()'
                DJB_CharacterInstance[len(DJB_CharacterInstance)-1].makeAnimDataJoints()
                print 'executing DJB_CharacterInstance[len(DJB_CharacterInstance)-1].makeControls()'
                DJB_CharacterInstance[len(DJB_CharacterInstance)-1].makeControls()
                print 'executing DJB_CharacterInstance[len(DJB_CharacterInstance)-1].hookUpControls()'
                DJB_CharacterInstance[len(DJB_CharacterInstance)-1].hookUpControls()
                print 'executing DJB_CharacterInstance[len(DJB_CharacterInstance)-1].writeInfoNode()'
    ...
    def browseMotions_function(self, arg = None):
        mayac.Import()
    def browseMotionsRef_function(self, arg = None):
    ...
    def exportBakedSkeleton_function(self, arg = None):
        ...
        if DJB_CharacterInstance:
            ...
            DJB_CharacterInstance[0].createExportSkeleton(keepMesh_ = keepMesh)
