...
def ruSetBendAxis(args):
    bendAxis=mc.radioButtonGrp('ruBendAxis',q=True,sl=True)
    selectedJoint = mc.ls(sl=True,type='transform')
    jointOrient =mc.getAttr((str(selectedJoint[0])+'.jot')) # mc.joint(selectedJoint,q=True, roo=True)
    print jointOrient

def ruRenameFinger(rootChain,newnametemplate,finger,indexNumber):
    children =  mc.listRelatives(rootChain, children=True, type='transform',path=True)
    ...
    if children:
        for child in children:
            indexNumber += 1
            ruRenameFinger(child,newnametemplate,finger,indexNumber)


def ruRenameFingerChain():
    root = mc.ls(sl=True, type='transform')
    finger = mc.optionMenu('ruFinger',q=True,value=True)
    template=mc.textFieldButtonGrp('ruTemplateName',q=True,text=True)
    
    ruRenameFinger(str(root[0]),template,finger,0)
...
def ruMainWindow():
	...
	mc.columnLayout(rs=5,adj=1)
	mc.radioButtonGrp('ruBendAxis', numberOfRadioButtons=3, onc=ruSetBendAxis, label='Bend Around', labelArray3=['X', 'Y', 'Z'],cw4=[100,50,50,50],ct4=['left', 'left','left','left'],co4=[0,0,0,0])
	...
	mc.menuItem(l='Ring')
	mc.menuItem(l='Pinky')
	mc.textFieldButtonGrp('ruTemplateName', label='Template string', text='', buttonLabel='Rename', bc=ruRenameFingerChain, cw3=[120,70,70],ct3=['left','left','left'],co3=[2,2,2] )
