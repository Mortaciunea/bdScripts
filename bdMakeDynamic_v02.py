...
class bdMakeDynamic():
	...
	def bdCreateDynChain(self):
		...
		if self.hairSystem == '':
			...
			pm.mel.eval('assignNewHairSystem')
			print drvCrvShape
			...
			try:
				flcShape = pm.listConnections('%s.local' % drvCrvShape, s=False )[0]
	...
	def bdCreateFkDynSwitch(self):
		...
		for cnstr  in parentCnstrAll:
			dynAttrW = pm.listAttr(cnstr, st='*DYN*')[0]
			fkAttrW = pm.listAttr(cnstr, st='*FK*')[0]
...
class bdMakeDynamicUI(bdMakeDynamicUI_form,bdMakeDynamicUI_base):
	...
	def create_connections(self):
		self.addDriverChainsBtn.clicked.connect(self.bdAddDynamicChain)
	...
	def bdAddDynamicChain(self):
		...
		if hairSystem == "New":
			...
			print 'adasdasdasdasdas', hairSystemName 
