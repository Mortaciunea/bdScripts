...
def bdRigQLegIk(side):
    ikAnimCon = pm.ls(side + '*foot*ik_ctrl',type='transform')[0]
    legBonesNames = ['thigh','knee','foot','toe']
    #START setup foot roll 
    #legIk = pm.ikHandle(sol= 'ikRPsolver',sticky='sticky', startJoint=bdGetBone(side,'thigh','ikpv'),endEffector = bdGetBone(side,'foot','ikpv'),name = side + '_leg_ikHandle')[0]
    
    ankleIk = pm.ikHandle(sol= 'ikRPsolver',sticky='sticky', startJoint=bdGetBone(side,'thigh','ik'),endEffector = bdGetBone(side,'ankle','ik'),name = side + '_ankle_ikHandle')[0]
    footIk  = pm.ikHandle(sol= 'ikSCsolver',sticky='sticky', startJoint=bdGetBone(side,'ankle','ik'),endEffector = bdGetBone(side,'foot','ik'),name = side + '_foot_ikHandle')[0]
    ballIk = pm.ikHandle(sol= 'ikSCsolver',sticky='sticky', startJoint=bdGetBone(side,'foot','ik'),endEffector = bdGetBone(side,'toe','ik'),name = side + '_ball_ikHandle')[0]
    toeIk = pm.ikHandle(sol= 'ikSCsolver',sticky='sticky', startJoint=bdGetBone(side,'toe','ik'),endEffector = bdGetBone(side,'foot','ik_end'),name = side + '_toe_ikHandle')[0]
    
    
    #create the groups that will controll the foot animations ( roll, bend, etc etc)
    footHelpers = pm.ls(side + '*_helper',type='transform')

    legLoc = bdCreateOffsetLoc(bdGetBone(side,'foot','ik'),side + '_leg_loc')
    ankleLoc = bdCreateOffsetLoc(bdGetBone(side,'foot','ik'),side + '_ankle_loc')
    #ankleLocParent = bdCreateOffsetLoc(bdGetBone(side,'foot','ik'),side + '_ankle_parent_loc')
    ankleLocSwing = bdCreateOffsetLoc(bdGetBone(side,'foot','ik'),side + '_ankle_swing_loc')
    footLoc = bdCreateOffsetLoc(bdGetBone(side,'foot','ik'),side + '_foot_loc')
    ballLoc = bdCreateOffsetLoc(bdGetBone(side,'toe','ik'),side + '_ball_loc')
    ballTwistLoc = bdCreateOffsetLoc(bdGetBone(side,'toe','ik'),side + '_ball_twist_loc')
    toeLoc = bdCreateOffsetLoc(bdGetBone(side,'foot','ik_end'),side + '_toe_loc')
    toeBendLoc = bdCreateOffsetLoc(bdGetBone(side,'toe','ik'),side + '_toe_bend_loc')
    ...
    pm.parent(heelLoc,legLoc)

    pm.parent(ankleIk,ankleLocSwing)
    pm.parent(ankleLocSwing,ankleLoc)
    pm.parent(ankleLoc,footLoc)
    
    #pm.parentConstraint(bdGetBone(side,'foot','ikpv'),ankleLoc)
    
    #add atributes on the footGrp - will be conected later to an anim controler
    autoRollAttrList = ['Roll','ToeStart','BallStraight']
    footAttrList = ['HeelTwist','BallTwist','TipTwist','Bank','ToeBend']
    normalRollAttrList = ['HeelRoll','BallRoll','TipRoll']

    pm.addAttr(ikAnimCon ,ln='autoFootRoll',nn='--------',at='enum',en = 'Auto Foot Roll' )
    ikAnimCon.attr('autoFootRoll').setKeyable(True)
    ikAnimCon.attr('autoFootRoll').setLocked(True)
    ...
    pm.addAttr(ikAnimCon ,ln='footRoll',nn='--------',at='enum',en = 'Foot Roll'  )
    ikAnimCon.attr('footRoll').setKeyable(True)
    ikAnimCon.attr('footRoll').setLocked(True)

    for attr in normalRollAttrList:
        pm.addAttr(ikAnimCon ,ln=attr,nn=attr,at='float' )
        ikAnimCon.attr(attr).setKeyable(True)

    pm.addAttr(ikAnimCon ,ln='footAttr',nn='--------',at='enum' ,en = 'Foot Attr')
    ikAnimCon.attr('footAttr').setKeyable(True)
    ikAnimCon.attr('footAttr').setLocked(True)
...
def bdGetBone(side,name,suffix):
    bone = pm.ls(side + '*' + name + '*' + suffix)[0]
    return bone
...
def bdConnectBank(ikAnimCon,innerLoc,outerLoc):
    print innerLoc, outerLoc
    outerLoc = pm.ls(outerLoc)[0]
    innerLoc = pm.ls(innerLoc)[0]
    side = 'left'
    if 'right' in ikAnimCon.name():
        side = 'right'
